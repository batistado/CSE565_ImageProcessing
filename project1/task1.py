...
def convolve2d(img, kernel):
    ...
    for i in range(len(convoluted_img)):
        for j in range(len(convoluted_img[0])):
            convoluted_img[i][j] = utils.add_matrix(utils.elementwise_mul(utils.crop(padded_img, i, i + 3, j, j + 3), kernel))
...
def normalize(img):
    ...
    normalized_img = copy.deepcopy(img)

    for i in range(len(img)):
        for j in range(len(img[0])):
            max_value = max(img[i][j], max_value)
            min_value = min(img[i][j], min_value)
...
def edge_magnitude(edge_x, edge_y):
    ...
    return normalize(np.sqrt(edge_x ** 2 + edge_y ** 2))
